define(['exports'], (function (exports) { 'use strict';
    ...
    */
    ...
    const isInstance$2 = (object, // Need the general type to do the check later.
    // eslint-disable-next-line @typescript-eslint/ban-types
    ...
    const isArrayOfClass$2 = (value, // Need general type to do check later.
    ...
    class PrecacheStrategy extends Strategy {
      ...
      async _handleFetch(request, handler) {
        ...
        if (this._fallbackToNetwork) {
          {
            logger$1.warn(`The precached response for ` + `${getFriendlyURL$1(request.url)} in ${this.cacheName} was not ` + `found. Falling back to the network.`);
          }
          ...
          if (integrityInManifest && noIntegrityConflict) {
            ...
            {
              if (wasCached) {
                logger$1.log(`A response for ${getFriendlyURL$1(request.url)} ` + `was used to "repair" the precache.`);
              }
            }
          }
        } else {
...

        {
          ...
          logger$1.log(`Serving the precached url: ${getFriendlyURL$1(cacheKey instanceof Request ? cacheKey.url : cacheKey)}`);
    ...
    */
    ...
    function cleanupOutdatedCaches() {
      ...
      self.addEventListener('activate', event => {
        const cacheName = cacheNames$1.getPrecacheName();
    ...
    */
    ...
    class NavigationRoute extends Route {
      ...
      constructor(handler, {
        ...
        {
          finalAssertExports$2.isArrayOfClass(allowlist, RegExp, {
            moduleName: 'workbox-routing',
            className: 'NavigationRoute',
            funcName: 'constructor',
            paramName: 'options.allowlist'
          });
          finalAssertExports$2.isArrayOfClass(denylist, RegExp, {
            moduleName: 'workbox-routing',
            className: 'NavigationRoute',
            funcName: 'constructor',
      ...
      _match({
        ...
        for (const regExp of this._denylist) {
          if (regExp.test(pathnameAndSearch)) {
            {
              logger$2.log(`The navigation route ${pathnameAndSearch} is not ` + `being used, since the URL matches this denylist pattern: ` + `${regExp.toString()}`);
            }
        ...
        if (this._allowlist.some(regExp => regExp.test(pathnameAndSearch))) {
          {
            logger$2.debug(`The navigation route ${pathnameAndSearch} ` + `is being used.`);
          }
        ...
        {
          logger$2.log(`The navigation route ${pathnameAndSearch} is not ` + `being used, since the URL being navigated to doesn't ` + `match the allowlist.`);
        }
